...
class SupervisorGA:
    def __init__(self):
        ...
        self.display.drawText("Fitness (Average - Green)", 0,100)
    ...
    def evaluate_genotype(self,genotype,generation):
        ...
        while currentInteraction < numberofInteractionLoops:
            ...
            robot_orie = self.robot_node.getField("rotation")
            values = robot_pose.getSFVec3f()
            rot_values = robot_orie.getSFRotation()
            angle = rot_values[3]
            robot_x = values[0]
            robot_y = values[2]
            reward_right = 1
            if robot_y > 0:
                reward_right = reward_right -1
            if robot_x > 0.25:
                reward_right = reward_right + 1
            #if angle < 1.50 and angle > 1.57:
                #reward_right = reward_right + 25
            ...
            reward_left = 1
            if robot_y > 0:
                reward_left = reward_left - 1
            if robot_x < - 0.25:
                reward_left = reward_left + 1 
 
            #if angle > 3 and angle < 3.14:
                #reward_left = reward_left + 25
            #reward_left = 0
            #if robot_x < 0:
                #reward_left = reward_left + abs(robot_x)
            #else:
                #reward_left = reward_left - abs(robot_x)
