...
class SupervisorGA:
    ...
    def handle_receiver(self):
        while(self.receiver.getQueueLength() > 0):
            ...
            elif(typeMessage == "leftvel"):
                self.receivedLeft = float(self.receivedData[9:len(self.receivedData)])
            elif(typeMessage == "rigtvel"):
                self.receivedRight = float(self.receivedData[9:len(self.receivedData)])
    ...
    def evaluate_genotype(self,genotype,generation):
        ...
        while currentInteraction < numberofInteractionLoops:
            ...
            left = self.receivedLeft
            right = self.receivedRight
            ...
            robot_y = values[2]
                ...
                #reward_right = 0
            if robot_x > 0:
                reward_right = 50 + abs(robot_x)
            else:
                reward_right = - abs(robot_x)
            #d = math.sqrt((target_right_x - robot_x)**2 + (target_right_y - robot_y)**2)
            ...
            if robot_x < 0:
                reward_left = 50 + abs(robot_x)
            else:
                reward_left = - abs(robot_x)
                
            #d = math.sqrt((target_left_x - robot_x)**2 + (target_left_y - robot_y)**2)
