...
class Controller:
    ...
    def calculate_fitness(self):
        ...
        spinningFitness = 1 - math.sqrt(abs(self.velocity_left-self.velocity_right))
        ### ground sensor
        turnFitness = 0
        if (sum(self.inputs[:3])/3) > 0.5:
            if (self.velocity_left < self.velocity_right):
                turnFitness = abs(self.velocity_left - self.velocity_right)
            elif (self.velocity_left > self.velocity_right):
                turnFitness = -1 * (self.velocity_left - self.velocity_right)
        else:
            if (self.velocity_left < self.velocity_right):
                turnFitness = (self.velocity_left - self.velocity_right)
            elif (self.velocity_left > self.velocity_right):
                turnFitness = self.velocity_left - self.velocity_right
    
         ### Define the fitness function to stop robot
        #stopFitness = 1 - abs(self.velocity_left + self.velocity_right)/2
        ### Define the fitness function of this iteration which should be a combination of the previous functions         
        combinedFitness = (forwardFitness * avoidCollisionFitness * spinningFitness) * turnFitness
