...
class Controller:
    ...
    def calculate_fitness(self):
        ...
        avoidCollisionFitness = 1 - (self.inputs[3] + self.inputs[4] + self.inputs[9] + self.inputs[10])
        ...
        if (self.inputs[5] < 0.1 and self.inputs[8] < 0.1):
            if (self.velocity_left < self.velocity_right):
                    turnFitness = self.velocity_right - self.velocity_left
    ...
    def handle_receiver(self):
        if self.receiver.getQueueLength() > 0:
            while(self.receiver.getQueueLength() > 0):
                ...
                typeMessage = self.receivedData[0:5]
                if(typeMessage == "reset"):
                    self.hint = 0
